...
public class DownloadTokenCleanUp extends Job {
	...
	public void runJob() {
		...
		// [FilesDownloadServlet.DOWNLOADTOKEN]ABC123XYZ.1273628883 = /my/file.txt
		List<String[]> list = KVMemoryMap.getInstance().readListWithPartKey(FilesDownloadServlet.DOWNLOADTOKEN);
		for (String[] strings : list) {
			...
			if ((expire != 0 && expire < now) || !(new FilesFile(strings[1])).exists()) {
				KVMemoryMap.getInstance().deleteKey(FilesDownloadServlet.DOWNLOADTOKEN + strings[0]);
			}
		}
